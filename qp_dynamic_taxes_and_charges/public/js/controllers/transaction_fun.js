erpnext.TransactionController = erpnext.taxes_and_totals.extend({
					...
					if(flag_add){
						...
						if(is_check_merge  && (cruce_de_impuestos_en_compras ? true : !['Purchase Order', 'Purchase Invoice', 'Purchase Receipt'].includes(me.frm.doc.doctype))){
							if (['On Previous Row Amount', 'Previous Row Total'].includes(child.charge_type)){
								let prev_account_head = item_tax_list.find(x => x.idx == item_tax_list.find(x => x.account_head == tax).row_id).account_head
								is_check_merge  && (cruce_de_impuestos_en_compras ? true : !['Purchase Order', 'Purchase Invoice', 'Purchase Receipt'].includes(me.frm.doc.doctype)) ? child.row_id = me.frm.doc.taxes.find(x => x.account_head == prev_account_head).idx : child.row_id = item_tax_list.find(x => x.account_head == tax).row_id;
							}
						}
					}
				}
	...
	// *** MENTUM: PROPUESTA PARA ASIGNAR CUENTA CONTRAPARTIDA

	autofill_expense_account : function (child_table, account_field, qp_account) {
		if (qp_account && child_table && child_table.length) {
			let doctype = child_table[0].doctype;
			$.each(child_table || [], function(i, item) {
				frappe.model.set_value(doctype, item.name, account_field, qp_account);
			});
		}
	},
